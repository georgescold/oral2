...
export function frameFormatExtension(format) {
    return format === "jpeg" ? "jpg" : format;
}
export function frameFormatMimeType(format) {
    return `image/${format}`;
}
function frameQualityArgs(format) {
    if (format === "jpeg")
        return ["-q:v", "5"];
    if (format === "webp")
        return ["-quality", "80"];
...
function isMissingWebpEncoderError(error) {
    ...
    return (message.includes("codec webp") &&
...
export async function extractFrames(videoPath, options) {
    const { fps, resolution, outputDir, format = "jpeg", startTime, endTime, maxFrames = 100, } = options;
    ...
    args.push("-vf", `fps=${fps},scale=${resolution}:-1`, "-frames:v", String(maxFrames), ...frameQualityArgs(format), join(outputDir, `frame_%04d.${extension}`));
    try {
        await execFileAsync("ffmpeg", args);
    }
    catch (error) {
        if (format === "webp" && isMissingWebpEncoderError(error)) {
            throw new Error("WebP frame extraction requires an ffmpeg build with a WebP encoder (libwebp). " +
                "Use frame_format='jpeg' or frame_format='png', or install an ffmpeg build such as ffmpeg-full.", { cause: error });
        }
        throw error;
    }
 ...
 */
export async function extractFramesBySegments(videoPath, segments, baseOutputDir, format = "jpeg") {
    const allFrames = [];
