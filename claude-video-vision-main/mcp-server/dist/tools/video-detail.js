...
// Helpers
// ---------------------------------------------------------------------------
function timestampToFormattedFilename(ts, format) {
    return `${ts.replace(/:/g, "-")}.${frameFormatExtension(format)}`;
}
function mimeTypeFromFrameFile(file) {
 ...
 */
function buildViewablePool(extractedFrames, manifest, format) {
 ...
 */
function lookupTimestampsInManifest(manifest, timestamps, format) {
    const result = [];
...
export function registerVideoDetail(server) {
    server.tool("video_detail", "Drill into specific segments of a local video file or YouTube URL. Extracts frames at variable FPS/resolution per segment. Separates extraction from viewing: use segments to extract, view/view_sample to control which frames are returned as images. When enable_index is on, frames are cached and deduplicated across calls.", {
        ...
        if (params.segments && params.segments.length > 0) {
            ...
            try {
                extractedFrames = await extractFramesBySegments(safePath, params.segments, tmpWorkDir, frameFormat);
            }
            finally {
            ...
            if (config.enable_index && manifest && sessionDir) {
                ...
                for (const [resolution, frames] of byResolution) {
                    ...
                    for (const frame of frames) {
                        const filePath = join(resDir, timestampToFormattedFilename(frame.timestamp, frameFormat));
