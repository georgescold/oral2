...
function cachePrefixForUrl(url) {
...
export function cleanExpiredDownloads(downloadsDir, maxAgeDays) {
...
function findDownloadedPath(stdout) {
    const lines = stdout
        .split(/\r?\n/)
        .map((line) => line.trim())
        .filter(Boolean);
...
function chooseCaptionTrack(data) {
...
async function fetchYouTubeInfo(url) {
    try {
        const { stdout } = await execFileAsync("yt-dlp", ["--skip-download", "--no-playlist", "--dump-single-json", url], {
            timeout: 30_000,
...
async function fetchYouTubeCaptions(url, captionTrack) {
    if (!captionTrack)
        return null;
...
export function getTranscriptCoverageSeconds(transcription) {
    let maxEnd = 0;
...
export function getCaptionFallbackReason(captions, durationSeconds) {
    if (!captions)
        return "no YouTube caption track found";
    if (captions.transcription.length === 0)
        return "YouTube caption track was empty";
...
export function buildCaptionAudioResult(captions, range) {
...
export async function resolveVideoInputDetailed(input) {
...
export async function resolveVideoInput(input) {
