...
import { planChunks } from "../extractors/audio-chunker.js";
...
function offsetTimestamp(hms, offsetSec) {
    return formatHMS(parseHMS(hms) + offsetSec);
}
export async function transcribeChunk(wavPath, offsetSec, config) {
...
export async function transcribeChunkWithRetry(wavPath, offsetSec, config, retries, worker = transcribeChunk, onWarning) {
    for (let attempt = 0; attempt <= retries; attempt++) {
        try {
            ...
            if (attempt < retries) {
                if (onWarning)
                    onWarning({ event: "retry", attempt, error: msg });
                continue;
            }
...
export async function analyzeWithGeminiApi(videoPath, config, slice, deps = {}) {
    ...
    try {
        ...
        // Chunked path
        const { chunks, warnings: planWarnings } = await planChunks(videoPath, metadata.duration_seconds, config, silenceDetector);
        const wavPaths = await Promise.all(chunks.map(c => extract(videoPath, tmpDir, {
            startTime: secondsToHMS(c.actual_start),
            endTime: secondsToHMS(c.end),
            filename: `chunk-${c.index}.wav`,
        })));
        const allWarnings = [...planWarnings];
        const results = await Promise.all(chunks.map((c, i) => transcribeChunkWithRetry(wavPaths[i], c.start, config, 1, worker, (w) => {
            allWarnings.push({
                chunk_index: c.index,
                chunk_total: c.total,
        ...
        chunks.forEach((c, i) => {
            ...
            if (r.ok) {
                for (const seg of r.segments ?? []) {
                    ...
                    if (clamped)
                        transcription.push(clamped);
                }
                for (const tag of r.tags ?? []) {
                    const clamped = clampSegment(tag, c.start, c.end);
                    if (clamped)
                        audio_tags.push(clamped);
                }
            }
            else {
                transcription.push({
...
function clampSegment(seg, minSec, maxSec) {
...
function hmsRange(startSec, endSec) {
